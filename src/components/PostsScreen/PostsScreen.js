...
import BadgesSearch from './PostsSearch'
...
class PostsScreen extends React.Component{
    ...
    //It makes a request of all the existen posts.
    fetchdata = async() => {
        console.log('Fetching data');
        this.setState({loading: true});
        let response = await Http.instance.get_all();
        this.setState({loading: false, posts: response, postscopy: response});
    };

    //It sends the user to the details of the badge.
    handlePress = item => {
        this.props.navigation.navigate('PostsDetail', {item});
    };
    ...
    handleChange = query => {
        ...
        const postsFiltered = postscopy.filter( badge => {
        ...
        Alert.alert(
            ...
            [
                ...
                },
                {
                    text: 'Delete',
                    onPress: async () => {
                        this.setState({loading: true, posts: undefined});
